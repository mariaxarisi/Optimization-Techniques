...
for s = 1:3
    ...
    % ---- 1) Fixed Step ----
    [xs_fix, fs_fix, k_fix] = steepest_descent_fixedstep(f_wr, gf_wr, x0, gamma_fixed, epsilon, maxiter);
    fprintf("-----------Fixed Step-----------\n")
    fprintf("Iterations: %d\n", k_fix);
    fprintf("Mininum f(%.6f, %.6f) = %.6f\n", xs_fix(1, end), xs_fix(2, end), fs_fix(end))

    % ---- 2) Optimal Step ----
    [xs_opt, fs_opt, k_opt] = steepest_descent_optimal(f_wr, gf_wr, x0, epsilon, maxiter);
    fprintf("-----------Optimal Step-----------\n")
    fprintf("Iterations: %d\n", k_opt);
    fprintf("Mininum f(%.6f, %.6f) = %.6f\n", xs_opt(1, end), xs_opt(2, end), fs_opt(end))

    % ---- 3) Armijo Step ----
    [xs_arm, fs_arm, k_arm] = steepest_descent_armijo(f_wr, gf_wr, x0, gamma_fixed, 0.01, 0.3, epsilon, maxiter);
    fprintf("-----------Armijo Step-----------\n")
    fprintf("Iterations: %d\n", k_arm);
...
sgtitle('Steepest Descent: Convergence of objective function','FontSize',16)
...
function [xs, fs, k] = steepest_descent_optimal(f, gf, x0, epsilon, maxiter)
...
function [xs, fs, k] = steepest_descent_armijo(f, gf, x0, sigma, alpha, beta, epsilon, maxiter)
