...
for i = 1:length(ls)
    [~, ~, num_eval_l(i,1)] = fibonacci_method(f1, a, b, ls(i));
    [~, ~, num_eval_l(i,2)] = fibonacci_method(f2, a, b, ls(i));
    [~, ~, num_eval_l(i,3)] = fibonacci_method(f3, a, b, ls(i));
end
...
title('Fibonacci Method: Variation of evaluations with l');
...
for j = 1:length(L)
    ...
    [~, ~, ~, k1, ak1, bk1] = fibonacci_method(f1, a, b, l);
    [~, ~, ~, k2, ak2, bk2] = fibonacci_method(f2, a, b, l);
    [~, ~, ~, k3, ak3, bk3] = fibonacci_method(f3, a, b, l);
